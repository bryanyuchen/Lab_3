...
void loop() {
  ...
  Serial.println();
  if (val == 0) {
    state = !state; //flip state of switch if vibration sensor is triggered
  }
  delay(100); //prevent accidental triggering
  if (state == false) {
    startServer(); //activate webserver control if manual control is off
  }
  if (state == false) {
    if (message == "Sequence1") {
      playBeat(rhythm_1);
    }
    else if (message == "Sequence2") {
      playBeat(rhythm_2);
    }
    else if (message == "Sequence3") {
      playBeat(rhythm_3);
    }
  }
  else {
  }
  // initialize rate of beat for this machine (beats twice as fast as previous machine input)
